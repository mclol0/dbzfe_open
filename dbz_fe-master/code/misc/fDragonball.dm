var/dragonballDatum/DBDatum = new();
...
dragonballDatum
	...
	proc
		...
		scatterDBS(){
			for(var/obj/item/DRAGONBALLS/d in dragonballs){
				d.scatter();
			}
		}
		...
		checkForDB(mob/Player/m=NULL){ //Check if we have all 7 in the same room
			...
			var/obj/item/DRAGONBALLS/dLoc = dragonballs[1];
		...
		listenForCaller(){
			...
			while(src){
				if(!WISHING && areActive() && checkForDB()){
					...
					send("{YThe Drag({x{R*{x{Y)n{RBalls{x {Yare glowing!{x",_ohearers(0,dLoc));
				}
				sleep(10 SECONDS);
			}
		}
...
obj/item/DRAGONBALLS
	...
	ONE_STAR_DRAGONBALL
		DISPLAY = "{YOne Star Drag({x{R*{x{Y)n{x{RBall{x"
		SHOW_ITEMDB = FALSE;

	TWO_STAR_DRAGONBALL
		DISPLAY = "{YTwo Star Drag({x{R*{x{Y)n{x{RBall{x"
		SHOW_ITEMDB = FALSE;

	THREE_STAR_DRAGONBALL
		DISPLAY = "{YThree Star Drag({x{R*{x{Y)n{x{RBall{x"
		SHOW_ITEMDB = FALSE;

	FOUR_STAR_DRAGONBALL
		DISPLAY = "{YFour Star Drag({x{R*{x{Y)n{x{RBall{x"
		SHOW_ITEMDB = FALSE;

	FIVE_STAR_DRAGONBALL
		DISPLAY = "{YFive Star Drag({x{R*{x{Y)n{x{RBall{x"
		SHOW_ITEMDB = FALSE;

	SIX_STAR_DRAGONBALL
		DISPLAY = "{YSix Star Drag({x{R*{x{Y)n{x{RBall{x"
		SHOW_ITEMDB = FALSE;

	SEVEN_STAR_DRAGONBALL
		DISPLAY = "{YSeven Star Drag({x{R*{x{Y)n{x{RBall{x"
		SHOW_ITEMDB = FALSE;
