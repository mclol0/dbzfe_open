...
CGI
	...
	Topic(){
		var
			...
			players = world.Export("byond://dbzfe.com:4000?action=players");
			onlinePlayers = world.Export("byond://dbzfe.com:4000?action=player_count");
			//header = "<title>Drag(*)nBall Z: (FE) (ALPHA)</title><body bgcolor=#000000>"
			//footer = "</body>"
			content = "[mudName] {WOnline Players{x[breakL(2)][players][breakL(2)]{R[onlinePlayers]{x {Wplayer(s) online.{x"
