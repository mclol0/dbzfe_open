Command/Technique
	omegacannon
		name = "omegacannon"
		internal_name = "omegacannon"
		format = "~omegacannon; ?~searc(mob@planet)";
		syntax = list("mobile")
		priority = 1;
		_maxDistance = 8;
		_minDistance = 4;
		...
		helpDescription = "Chargeable wave of energy that shoots from your mouth."
		skillDatum = /fEAttk/omegacannon
		...
		command(mob/user, mob/target=user:fCombat._getLastTarget()) {
			...
			if(user.kiAttk){
				...
				if(target && a_get_dist(user,target) <= _maxDistance && a_get_dist(user,target) >= _minDistance || target && a_get_dist(user,target) <= _maxDistance && (target in user.fCombat.hostileTargets) || !target){
					...
					send("{B* You open your mouth as {Genergy{x{B gathers...\n{x",user)
					send("{W*{x [user.raceColor(user.name)] opens [user.determineSex(1)] mouth and gathers {Genergy{x{W...\n",_ohearers(0,user))
					user.kiAttk = new /fEAttk/omegacannon(user)
				}else{
					syntax(user, getSyntax());
				}
			}
