...
mob
	cClient
		proc
			...
			loadCharacter(name,COPYOVER=FALSE){
				var
					loadVariables = list("race",
										"sex",
										"alignment",
										"currpl",
										"maxpl",
				...
				if(COPYOVER){ send(" {Y({x{R*{x{Y){x  And before you know it, everything has changed  {Y({x{R*{x{Y){x ",m,TRUE); }
	...
	Player
		proc
			...
			saveSQLCharacter(){
				...
				if(rowCount > 0){
					_query("UPDATE `characters` SET `visuals`='[list2params(visualParams)]', `stats`='[list2params(saveParams)]', `equipment`='[list2params(eqParams)]', `techniques`='[list2params(cParams)]', `channels`='[list2params(channels)]' WHERE `name`='[name]' COLLATE NOCASE;")
				}else{
					_query("INSERT INTO `characters` (`name`, `email`, `password`, `visuals`, `stats`, `equipment`, `techniques`, `channels`, `wizlevel`) VALUES ('[name]', '[email]', '[password]', '[list2params(visualParams)]', '[list2params(saveParams)]', '[list2params(eqParams)]', '[list2params(cParams)]', '[list2params(channels)]', '0');")
				}
