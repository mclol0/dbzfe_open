...
obj/item/
		...
		decayItem(oldLoc){
			...
			while(loc == oldLoc){
				if(world.time >= decayTime){
					send("[DISPLAY] decays.",oview(0,src))
					clean() // del replace
					break;
				}
	...
	proc
		_open(mob/m){
			if(CONTAINER){
				send("You open [DISPLAY].",m);
				send("[m.raceColor(m.name)] opens [DISPLAY].",_ohearers(0,m));
				return TRUE;
			}else{
		...
		_eat(mob/m){
			if(EDIBLE || isplayer(m) && m.isImm){
				send("You eat [DISPLAY].",m)
				send("[m.raceColor(m.name)] eats [DISPLAY].",_ohearers(0,m))
				return TRUE;
			}
		...
		_smoke(mob/m){
			if(SMOKEABLE || isplayer(m) && m.isImm){
				send("You smoke [DISPLAY].",m)
				send("[m.raceColor(m.name)] smokes [DISPLAY].",_ohearers(0,m))
				return TRUE;
			}
		...
		_equip(mob/m){
			...
			send("You equip [DISPLAY] [EQ_MSG] your [_getName(Sl)]!",m)
			send("[m.raceColor(m.name)] equip's [DISPLAY] [EQ_MSG] [m.determineSex(1)] [_getName(Sl)]!",_ohearers(0,m))
			m.remInv(src)
			loc = NULL;
			m.equipment[Sl] = src
		...
		_unequip(mob/m, var/mute=FALSE){
			...
			if(m.equipment[_SLOT]){
				if (!mute) {
					send("You unequip [DISPLAY] from your [_getName(_SLOT)]!",m)
					send("[m.raceColor(m.name)] unequip's [DISPLAY] from [m.determineSex(1)] [_getName(_SLOT)]!",_ohearers(0,m))
				}
		...
		stash(mob/m, obj/item/I) {
			send("You cannot put [I.PREFIX][I.DISPLAY] in [DISPLAY]", m);
		}
...
/* Jewelry/Rings */
		...
		_open(mob/Player/m){
			if(..(m)){
				if(prob(10) && m.maxpl < MAX_PL){
					...
					m.addInv(itemx);
					send("You obtain [itemx.PREFIX][itemx.DISPLAY] from [DISPLAY].",m)
					send("[m.raceColor(m.name)] obtains [itemx.PREFIX][itemx.DISPLAY] from [DISPLAY].",_ohearers(0,m))
					if(itemx.ANNOUNCE_DROP){ send("{R\[{x{YWORLD{x{R\]{x [itemx.PREFIX][itemx.DISPLAY] dropped from [DISPLAY] opened by [m.raceColor(m.name)]!",game.players); }
				}
				return TRUE;
			}
			return FALSE;
	...
	BLACK_STAR_LENSES_OF_MALICE //
		EQUIPABLE = TRUE;
		PREFIX = "the ";
		DISPLAY = "{DBl{x{ba{x{Dck{x {WS{x{Dta{x{Wr{x {DL{x{be{x{rns{x{be{x{Ds{x {cof{x {DM{x{Wa{x{rli{x{Wc{x{De{x"
		MULTI = FALSE;
		SLOT = EYE;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	// NEW DRAGON BALL/Z THEMED ITEMS
	VEGETAS_SAIYAN_PRIDE_ARMOR
		EQUIPABLE = TRUE;
		DISPLAY = "{cVegeta's{x {RSaiyan{x {mPride{x {WArmor{x"
		MULTI = FALSE;
		SLOT = CHEST;
		STACKABLE = TRUE;
		EQ_MSG = "over";
	...
	KRILLINS_DESTRUCTO_DISC_BRACERS
		EQUIPABLE = TRUE;
		DISPLAY = "{yKrillin's{x {WDestructo{x {YDisc{x {wBracers{x"
		MULTI = FALSE;
		SLOT = WRISTS;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	YAMCHAS_WOLF_FANG_GAUNTLETS
		EQUIPABLE = TRUE;
		DISPLAY = "{yYamcha's{x {DWolf{x {WFang{x {DGauntlets{x"
		MULTI = FALSE;
		SLOT = HANDS;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	TIENS_THIRD_EYE_HEADBAND
		EQUIPABLE = TRUE;
		DISPLAY = "{yTien's{x {WThird{x {rEye{x {yHeadband{x"
		MULTI = FALSE;
		SLOT = HEAD;
		STACKABLE = TRUE;
		EQ_MSG = "around";
	...
	NAPPAS_BATTLE_GAUNTLETS
		EQUIPABLE = TRUE;
		DISPLAY = "{cNappa's{x {RBattle{x {DGauntlets{x"
		MULTI = FALSE;
		SLOT = HANDS;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	TURTLE_HERMIT_SHELL
		EQUIPABLE = TRUE;
		DISPLAY = "{gTurtle{x {yHermit{x {gShell{x"
		MULTI = FALSE;
		SLOT = SHOULDERS;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	CAPTAIN_GINYUS_BODY_CHANGING_MEDALLION
		EQUIPABLE = TRUE;
		DISPLAY = "{mCaptain{x {YGinyu's{x {mBody{x {WChanging{x {YMedallion{x"
		MULTI = FALSE;
		SLOT = NECK;
		STACKABLE = TRUE;
		EQ_MSG = "around";
	...
	FRIEZAS_BATTLE_ARMOR_FRAGMENTS
		EQUIPABLE = TRUE;
		DISPLAY = "{WFrieza's{x {RBattle{x {WArmor{x {DFragments{x"
		MULTI = FALSE;
		SLOT = CHEST;
		STACKABLE = TRUE;
		EQ_MSG = "over";
	...
	GOHAN_SCHOLAR_SPECTACLES
		EQUIPABLE = TRUE;
		DISPLAY = "{cGohan's{x {WScholar{x {ySpectacles{x"
		MULTI = FALSE;
		SLOT = EYE;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	SUPREME_KAIS_POTARA_EARRINGS
		EQUIPABLE = TRUE;
		PREFIX = "a pair of ";
		DISPLAY = "{CSupreme{x {mKai's{x {YPotara{x {WEarrings{x"
		SINGLE_DISPLAY = "{CSupreme{x {mKai's{x {YPotara{x {WEarring{x"
		MULTI_DISPLAY = "{CSupreme{x {mKai's{x {YPotara{x {WEarring{x"
		MULTI = TRUE;
		SLOT = EARS;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	BULMAS_CAPSULE_CORP_JACKET
		EQUIPABLE = TRUE;
		DISPLAY = "{CBulma's{x {YCapsule{x {BCorp{x {CJacket{x"
		MULTI = FALSE;
		SLOT = CHEST;
		STACKABLE = TRUE;
		EQ_MSG = "over";
	...
	CHAIOTZU_EMPEROR_HAT
		EQUIPABLE = TRUE;
		DISPLAY = "{yChiaotzu's{x {YEmperor{x {rHat{x"
		MULTI = FALSE;
		SLOT = HEAD;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	VIDELS_FIGHTING_GLOVES
		EQUIPABLE = TRUE;
		DISPLAY = "{yVidel's{x {RFighting{x {wGloves{x"
		MULTI = FALSE;
		SLOT = HANDS;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	GREAT_SAIYAMANS_CAPE
		EQUIPABLE = TRUE;
		DISPLAY = "{cGreat{x {YSaiyaman's{x {GCape{x"
		MULTI = FALSE;
		SLOT = SHOULDERS;
		STACKABLE = TRUE;
		EQ_MSG = "over";
	...
	BABIDIS_MAJIN_EMBLEM
		EQUIPABLE = TRUE;
		DISPLAY = "{mBabidi's{x {MMajin{x {REmblem{x"
		MULTI = FALSE;
		SLOT = SPECIAL_HEAD;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	ANDROID_16S_BIRD_LOVING_INSIGNIA
		EQUIPABLE = TRUE;
		DISPLAY = "{DAndroid 16's{x {gBird{x {WLoving{x {DInsignia{x"
		MULTI = FALSE;
		SLOT = BACK;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	consecrated_ground
		EQUIPABLE = FALSE;
		DISPLAY = "{WConsecrated{x {gGround{x"
		SINGLE_DISPLAY = "{WConsecrated{x {gGround{x"
		MULTI_DISPLAY = "{WConsecrated{x {gGround{x"
		MULTI = FALSE;
		SLOT = NULL;
		STACKABLE = FALSE;
		WEIGHT = 0.0
	...
	ANDROID_PART
		...
		ANDROID_ACTUATOR
			EQUIPABLE = TRUE;
			DISPLAY = "{cAndroid{x {YActuator{x"
			SINGLE_DISPLAY = "{cAndroid{x {YActuator{x"
			MULTI_DISPLAY = "{cAndroid{x {YActuator{x"
			MULTI = FALSE;
			SLOT = NULL;
			STACKABLE = TRUE;
			WEIGHT = 1.0
		...
		ANDROID_SERVO
			EQUIPABLE = TRUE;
			DISPLAY = "{cAndroid{x {CServo{x"
			SINGLE_DISPLAY = "{cAndroid{x {CServo{x"
			MULTI_DISPLAY = "{cAndroid{x {CServo{x"
			MULTI = FALSE;
			SLOT = NULL;
			STACKABLE = TRUE;
			WEIGHT = 1.0
		...
		ANDROID_EYE
			EQUIPABLE = TRUE;
			DISPLAY = "{cAndroid{x {rEye{x"
			SINGLE_DISPLAY = "{cAndroid{x {rEye{x"
			MULTI_DISPLAY = "{cAndroid{x {rEye{x"
			MULTI = FALSE;
			SLOT = NULL;
			STACKABLE = TRUE;
			WEIGHT = 0.5
		...
		ANDROID_LEG
			EQUIPABLE = TRUE;
			DISPLAY = "{cAndroid{x {DLeg{x"
			SINGLE_DISPLAY = "{cAndroid{x {DLeg{x"
			MULTI_DISPLAY = "{cAndroid{x {DLeg{x"
			MULTI = FALSE;
			SLOT = NULL;
			STACKABLE = TRUE;
			WEIGHT = 2.0
		...
		ANDROID_ARM
			EQUIPABLE = TRUE;
			DISPLAY = "{cAndroid{x {wArm{x"
			SINGLE_DISPLAY = "{cAndroid{x {wArm{x"
			MULTI_DISPLAY = "{cAndroid{x {wArm{x"
			MULTI = FALSE;
			SLOT = NULL;
			STACKABLE = TRUE;
			WEIGHT = 1.5
	...
	// Additional Dragon Ball/Z/Super Items that were missing
	CELLS_PERFECT_FORM_EXOSKELETON
		EQUIPABLE = TRUE;
		DISPLAY = "{GCell's{x {MPerfect{x {GForm{x {WExoskeleton{x"
		MULTI = FALSE;
		SLOT = CHEST;
		STACKABLE = TRUE;
		EQ_MSG = "over";
	...
	VEGITO_POTARA_FUSION_BOOTS
		EQUIPABLE = TRUE;
		DISPLAY = "{cVegito's{x {YPotara{x {DFusion{x {WBoots{x"
		MULTI = FALSE;
		SLOT = FEET;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	ULTRA_INSTINCT_AURA_CLOAK
		EQUIPABLE = TRUE;
		DISPLAY = "{WUltra{x {CInstinct{x {WAura{x {cCloak{x"
		MULTI = FALSE;
		SLOT = SHOULDERS;
		STACKABLE = TRUE;
		EQ_MSG = "around";
	...
	WHIS_ANGEL_STAFF_REPLICA
		EQUIPABLE = TRUE;
		DISPLAY = "{CWhis'{x {WAngel{x {YStaff{x {wReplica{x"
		MULTI = FALSE;
		SLOT = HANDS;
		STACKABLE = TRUE;
		EQ_MSG = "in";
	...
	HIT_TIME_SKIP_GAUNTLETS
		EQUIPABLE = TRUE;
		DISPLAY = "{DHit's{x {mTime{x {WSkip{x {DGauntlets{x"
		MULTI = FALSE;
		SLOT = HANDS;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	KING_KAIS_ANTENNA_HEADBAND
		EQUIPABLE = TRUE;
		DISPLAY = "{mKing{x {CKai's{x {YAntenna{x {yHeadband{x"
		MULTI = FALSE;
		SLOT = HEAD;
		STACKABLE = TRUE;
		EQ_MSG = "around";
	...
	NAMEKIAN_ELDER_ROBES
		EQUIPABLE = TRUE;
		DISPLAY = "{gNamekian{x {WElder{x {cRobes{x"
		MULTI = FALSE;
		SLOT = CHEST;
		STACKABLE = TRUE;
		EQ_MSG = "over";
	...
	DENDE_HEALING_STAFF
		EQUIPABLE = TRUE;
		DISPLAY = "{gDende's{x {WHeal{x{Ging{x {YStaff{x"
		MULTI = FALSE;
		SLOT = HANDS;
		STACKABLE = TRUE;
		EQ_MSG = "in";
	...
	PICCOLOS_WEIGHTED_TRAINING_GI
		EQUIPABLE = TRUE;
		DISPLAY = "{gPiccolo's{x {wWeighted{x {gTraining{x {WGi{x"
		MULTI = FALSE;
		SLOT = CHEST;
		STACKABLE = TRUE;
		EQ_MSG = "over";
	...
	SAIYAN_TAIL_BELT
		EQUIPABLE = TRUE;
		DISPLAY = "{cSaiyan{x {DTail{x {yBelt{x"
		MULTI = FALSE;
		SLOT = WAIST;
		STACKABLE = TRUE;
		EQ_MSG = "around";
	...
	SUPER_SAIYAN_4_WRISTBANDS
		EQUIPABLE = TRUE;
		DISPLAY = "{RSuper{x {ySaiyan{x {R4{x {DWristbands{x"
		MULTI = FALSE;
		SLOT = WRISTS;
		STACKABLE = TRUE;
		EQ_MSG = "on";
	...
	BARDOCKS_BATTLE_WORN_HEADBAND
		EQUIPABLE = TRUE;
		DISPLAY = "{cBardock's{x {RBattle{x {DWorn{x {yHeadband{x"
		MULTI = FALSE;
		SLOT = HEAD;
		STACKABLE = TRUE;
		EQ_MSG = "around";
