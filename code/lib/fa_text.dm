proc
	...
	// fa_split the string into a list of substrings by fa_splitting it
	...
	// the case-sensitive version of fa_split()
	fa_Split(txt, d)
		#ifdef DEBUG
		ASSERT(istext(txt))
		ASSERT(istext(d))
	...
	// the opposite of fa_split - this converts a list to a single
	...
	// determines if p is a fa_prefix of txt
	fa_prefix(txt, p)
		return findtext(txt, p, 1, length(p) + 1)

	// the case-sensitive version of fa_prefix()
	fa_Prefix(txt, p)
