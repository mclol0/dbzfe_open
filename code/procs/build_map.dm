...
proc
	buildMap(mob/Player/user, _LEFT, _RIGHT, _TOP, _BOTTOM)
	{
		...
		var/list/RoomUsers = user.getRoom()
		var/list/RoomItems = user.getItemRoom()
		...
		if (user.loc && user.loc.loc)
		{
			for (var/YY in _TOP to _BOTTOM step -1)
			{
				for (var/XX in _LEFT to _RIGHT)
				{
					...
					{
						if (searcFor(T, user))
							map += user.map_mobMark(user)

						else if (M && M.visible && !M.invisible && isplayer(M) && user.insideBuilding == M.insideBuilding)
							map += user.map_mobMark(M)

						else if (M && M.visible && !M.invisible && isnpc(M) && user.insideBuilding == M.insideBuilding)
							map += user.map_mobMark(M)

						else if (X && X.visible)
							map += X.display

						else if (SP && SP.visible && SP:SHOW_IN_MAP && user.insideBuilding == SP.insideBuilding)
							map += SP.display

						else if (O && O.visible && O:SHOW_IN_MAP && user.insideBuilding == O.insideBuilding)
							map += O.display

						else if (T && T.visible)
						{
							if (length(T.possibleDisplay) > 0)
							{
								if (!user.insideBuilding)
									map += pick(T.possibleDisplay)
								else if (user.insideBuilding && houseSystem.isPlayerTurf(T) && T:instanceId == user.insideBuilding)
									map += pick(T.possibleDisplay)
								else
									map += " "
							}
		...
		buffer += compressMap(map, user.compression)
